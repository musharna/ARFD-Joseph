...
# BiocManager::install(c("DESeq2", "org.At.tair.db", "clusterProfiler", "GOSemSim", "apeglm"))
...
# =============================================================================
# OUTLIER REMOVAL - Matching Joseph's analysis
# =============================================================================
# Joseph removed the following samples based on PCA analysis:
# - GUS replicate 1 (sample 01)
# - GUS_aux replicate 3 (sample 16)
# - 5D replicate 2 (sample 10)
# - 6D replicate 2 (sample 11)
# - 7D replicate 3 (sample 19)
# - 8D replicate 3 (sample 20)
# - 19D replicate 3 (sample 21)

outlier_samples <- c("01", "10", "11", "16", "19", "20", "21")
sample_metadata <- sample_metadata[!sample_metadata$sample_num %in% outlier_samples, ]

cat("Samples after outlier removal:", nrow(sample_metadata), "\n")
cat("Removed samples:", paste(outlier_samples, collapse = ", "), "\n")
...
# Pre-filter low-count genes (matching Joseph's methodology)
...
# Using LFC shrinkage with apeglm (matching Joseph's methodology)
de_results_list <- list()

# Get the coefficient names from DESeq2
cat("Available coefficients:\n")
print(resultsNames(dds))

for (con in treatment_constructs) {
  # Get initial DESeq2 results (for pvalue, padj, baseMean)
  res <- results(dds, contrast = c("construct", con, control_construct),
                 independentFiltering = TRUE, alpha = 0.05, pAdjustMethod = "BH")

  # Construct the coefficient name for lfcShrink
  coef_name <- paste0("construct_", con, "_vs_", control_construct)

  # Apply LFC shrinkage using apeglm (if coefficient exists)
  if (coef_name %in% resultsNames(dds)) {
    lfc_shrunk <- lfcShrink(dds, coef = coef_name, type = "apeglm")
    # Use shrunken LFC values
    res$log2FoldChange <- lfc_shrunk$log2FoldChange
    res$lfcSE <- lfc_shrunk$lfcSE
    cat(sprintf("Applied apeglm shrinkage for %s\n", con))
  } else {
    cat(sprintf("Warning: Coefficient %s not found, using unshrunk LFC\n", coef_name))
  }
...
# Using baseMean > 20 filter (matching Joseph's methodology)
# baseMean is DESeq2's mean of normalized counts across ALL samples

for (con in treatment_constructs) {
  l2fc_col <- paste0("log2FoldChange-", con)
  padj_col <- paste0("padj-", con)
  basemean_col <- paste0("baseMean-", con)  # baseMean from DESeq2 results
  up_down_col <- paste0("Up_Down-", con)

  # Classify based on thresholds (matching Joseph's methodology):
  # - padj < threshold
  # - |log2FoldChange| > lfc_threshold
  # - baseMean > 20 (DESeq2's overall average across all samples)
  df2[[up_down_col]] <- ifelse(
    !is.na(df2[[padj_col]]) &
      df2[[padj_col]] < padj_threshold &
      abs(df2[[l2fc_col]]) > lfc_threshold &
      df2[[basemean_col]] > min_avg_reads,
...
if (file.exists(published_file)) {
  ...
  if (pub_lfc_6d %in% colnames(published_df) && cur_lfc_6d %in% colnames(df2)) {
    ...
    p_scatter <- ggplot(merged_6d, aes_string(x = paste0("`", pub_lfc_6d, "`"),
      ...
      theme_bw() +
      labs(
        title = "ARF6Δ: Published vs Current Log2 Fold Changes",
