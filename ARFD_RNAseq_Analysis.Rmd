...
joseph_file <- "ARF567 clusters.xlsx"

# Also try .xlxs in case of typo in filename
if (!file.exists(joseph_file) && file.exists("ARF567 clusters.xlxs")) {
  joseph_file <- "ARF567 clusters.xlxs"
}

if (file.exists(joseph_file) && "Cluster" %in% colnames(df2)) {
  ...
  if ("Geneid" %in% colnames(joseph_clusters) && "Gene_Cluster" %in% colnames(joseph_clusters)) {
    ...
    cluster_comparison <- df2 %>%
      dplyr::select(Geneid, Cluster) %>%
      filter(!is.na(Cluster)) %>%
      inner_join(
        joseph_clusters %>% dplyr::select(Geneid, Joseph_Cluster = Gene_Cluster),
        by = "Geneid"
      )

    cat("Genes found in both datasets:", nrow(cluster_comparison), "\n")
...
} else if (!file.exists(joseph_file)) {
  cat("Joseph's cluster file not found. Place 'ARF567 clusters.xlsx' in the working directory.\n")
} else {
