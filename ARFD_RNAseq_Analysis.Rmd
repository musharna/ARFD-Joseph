  ...
  for (p in pairs) {
    ...
    tryCatch({
      ...
      if (!is.null(full_res) && length(missed) > 0) {
        ...
        fr <- full_res %>%
          dplyr::mutate(Geneid_upper = toupper(trimws(Geneid))) %>%
          dplyr::filter(Geneid_upper %in% missed) %>%
        ...
        detail <- fr %>%
          dplyr::left_join(jlv, by = "Geneid_upper") %>%
          dplyr::mutate(
            joseph_lfc = as.numeric(joseph_lfc),
            Construct  = con_label,
            Category   = dplyr::case_when(
              is.na(my_lfc)                              ~ "Not in our data",
              is.na(my_padj) | my_padj > padj_threshold ~ "Padj too high",
              abs(my_lfc) < lfc_threshold                ~ "LFC below threshold",
              TRUE                                       ~ "Other"
            )
          )
      ...
      cat("  ERROR in", con_label, ":", conditionMessage(e), "\n")
    })
  }
