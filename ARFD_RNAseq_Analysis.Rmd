...
if (!is.na(joseph_file) && exists("joseph_clusters") &&
  ...
  # Map construct codes to display names
  construct_labels <- c(
    GUSaux_A = "GUS_aux", `5D_A` = "ARF5Δ", `6D_A` = "ARF6Δ",
    `7D_A` = "ARF7Δ",   `8D_A` = "ARF8Δ",  `19D_A` = "ARF19Δ"
  )
  ...
  for (con in treatment_constructs) {
    j_col  <- paste0("Up_Down-", con)
    j_lfc  <- paste0("log2FoldChange-", con)
    j_padj <- paste0("padj-", con)
    my_col <- paste0("Up_Down-", con)

    if (!j_col %in% colnames(joseph_clusters)) next
    ...
    recovery_rows[[con]] <- data.frame(
      Construct    = construct_labels[con],
