...
# min_avg_reads filter removed to match Joseph's pipeline (no equivalent filter)
...
# Compute Euclidean distance from centroid (PC1+PC2) to identify outliers
centroid <- colMeans(pca_result$x[, 1:2])
pca_df$dist_from_centroid <- sqrt(
  (pca_result$x[, 1] - centroid[1])^2 +
  (pca_result$x[, 2] - centroid[2])^2
)
pca_df_sorted <- pca_df[order(-pca_df$dist_from_centroid), ]
cat("Samples ranked by distance from PCA centroid (PC1+PC2):\n")
cat(sprintf("  %-12s  %-10s  %-10s  %s\n", "Sample", "Construct", "Distance", "Outlier?"))
cat(strrep("-", 52), "\n")
for (i in seq_len(nrow(pca_df_sorted))) {
  snum <- as.integer(pca_df_sorted$Sample[i])
  flag <- if (!is.na(snum) && snum %in% outlier_samples) " <-- OUTLIER" else ""
  cat(sprintf("  %-12s  %-10s  %10.2f  %s\n",
              pca_df_sorted$Sample[i], pca_df_sorted$Construct[i],
              pca_df_sorted$dist_from_centroid[i], flag))
}
```
...
# Pre-filter: remove genes with zero reads across all samples (matches Joseph's pipeline)
...
for (con in treatment_constructs) {
  # Get DESeq2 results
  res <- results(dds, contrast = c("construct", con, control_construct), alpha = 0.05)
...
for (con in treatment_constructs) {
  ...
  basemean_col <- paste0("baseMean-",        con)
  up_down_col  <- paste0("Up_Down-",         con)

  # Classify based on thresholds (matches Joseph's pipeline):
  # - padj < threshold
  # - |log2FoldChange| > lfc_threshold
  # - baseMean > 20 (matches Joseph's baseMean > 20 filter)
  df2[[up_down_col]] <- ifelse(
    !is.na(df2[[padj_col]]) &
      df2[[padj_col]] < padj_threshold &
      abs(df2[[l2fc_col]]) > lfc_threshold &
      !is.na(df2[[basemean_col]]) & df2[[basemean_col]] > 20,
...
# Load Joseph's reference file directly — does not depend on cluster_comparison chunk
deg_rec_candidates <- c(
  "ARFD_Up_Down_Regulated_LFC-1_p01_clustered(1).csv",
  "ARFD_Up_Down_Regulated_LFC-1_p01_clustered.csv",
  "ARF567 clusters.xlsx",
  "ARF567 clusters.xlxs"
)
deg_rec_file <- NA_character_
for (.f in deg_rec_candidates) {
  if (file.exists(.f)) { deg_rec_file <- .f; break }
}
if (!is.na(deg_rec_file)) {
  if (grepl("\\.csv$", deg_rec_file, ignore.case = TRUE)) {
    joseph_clusters_rec <- read.csv(deg_rec_file, check.names = FALSE, stringsAsFactors = FALSE)
  } else {
    library(readxl)
    joseph_clusters_rec <- as.data.frame(read_excel(deg_rec_file))
  }
} else {
  joseph_clusters_rec <- NULL
}

if (!is.null(joseph_clusters_rec) &&
    "Geneid" %in% colnames(joseph_clusters_rec) && exists("de_results_list")) {
  joseph_clusters <- joseph_clusters_rec
  ...
  # Hardcoded pairs: (our construct code, Joseph's column suffix, display label)
  pairs <- list(
    list(con="GUS_aux", jsuf="GUSaux_A", label="GUS_aux"),
    list(con="5D",      jsuf="5D_A",     label="ARF5"),
    list(con="6D",      jsuf="6D_A",     label="ARF6"),
    list(con="7D",      jsuf="7D_A",     label="ARF7"),
    list(con="8D",      jsuf="8D_A",     label="ARF8"),
    list(con="19D",     jsuf="19D_A",    label="ARF19")
  )
  # Only keep pairs whose construct is in this analysis
  pairs <- Filter(function(p) p$con %in% treatment_constructs, pairs)
  cat("Constructs to process:", paste(sapply(pairs, `[[`, "con"), collapse=", "), "\n\n")

  # Uppercase gene IDs from Joseph's file for consistent matching
  joseph_ids <- toupper(trimws(joseph_clusters$Geneid))
  ...
  for (p in pairs) {
    ...
    j_col  <- paste0("Up_Down-",        jsuf)
    j_lfc  <- paste0("log2FoldChange-", jsuf)
    j_padj <- paste0("padj-",           jsuf)
    my_col <- paste0("Up_Down-",        con)

    cat("Processing", con_label, "| j_col:", j_col, "| my_col:", my_col, "\n")

    if (!j_col %in% colnames(joseph_clusters)) {
      cat("  SKIP: column", j_col, "not found in Joseph's file\n")
      next
    }
    if (!my_col %in% colnames(df2)) {
      cat("  SKIP: column", my_col, "not found in df2\n")
      next
    }
    ...
    cat("  Joseph DEGs: UP=", length(j_up), "Down=", length(j_down), "\n")
    ...
    cat("  Our DEGs:    UP=", length(my_up), "Down=", length(my_down), "\n")

    missed <- setdiff(j_degs, my_degs)
    cat("  Missed:", length(missed), "\n")
    ...
    cat("  recovery_rows entries so far:", length(recovery_rows), "\n")

    # Categorise missed genes using full DESeq2 results
    full_res <- de_results_list[[con]]
    if (!is.null(full_res) && length(missed) > 0) {
      lfc_col_full  <- paste0("log2FoldChange-", con)
      padj_col_full <- paste0("padj-", con)

      fr <- full_res[toupper(trimws(full_res$Geneid)) %in% missed,
                     c("Geneid", lfc_col_full, padj_col_full)]
      colnames(fr) <- c("Geneid_upper", "my_lfc", "my_padj")
      fr$Geneid_upper <- toupper(trimws(fr$Geneid_upper))

      jlv <- joseph_clusters[toupper(trimws(joseph_clusters$Geneid)) %in% missed,
                              c("Geneid", j_lfc, j_padj)]
      colnames(jlv) <- c("Geneid_upper", "joseph_lfc", "joseph_padj")
      jlv$Geneid_upper <- toupper(trimws(jlv$Geneid_upper))
      jlv$joseph_lfc   <- as.numeric(jlv$joseph_lfc)

      detail <- merge(fr, jlv, by = "Geneid_upper", all.x = TRUE)
      detail$Construct <- con_label
      detail$Category  <- ifelse(
        is.na(detail$my_lfc),                                    "Not in our data",
        ifelse(is.na(detail$my_padj) | detail$my_padj > padj_threshold, "Padj too high",
        ifelse(abs(detail$my_lfc) < lfc_threshold,               "LFC below threshold",
                                                                  "Other")))
  ...
  # --- Summary table ---
  cat("recovery_rows length after loop:", length(recovery_rows), "\n")
  recovery_df <- do.call(rbind, lapply(recovery_rows, as.data.frame))
  rownames(recovery_df) <- NULL
  cat("recovery_df class:", class(recovery_df), "| nrow:", nrow(recovery_df), "\n\n")

  cat("=== DEG Recovery Summary ===\n")
  cat(sprintf("%-10s  %9s  %6s  %11s  %8s  %7s  %s\n",
              "Construct","Joseph_UP","My_UP","Joseph_Down","My_Down","Missed","Recovery%"))
  cat(strrep("-", 72), "\n")
  for (i in seq_len(nrow(recovery_df))) {
    cat(sprintf("%-10s  %9d  %6d  %11d  %8d  %7d  %.1f%%\n",
                recovery_df$Construct[i],
                recovery_df$Joseph_UP[i],   recovery_df$My_UP[i],
                recovery_df$Joseph_Down[i], recovery_df$My_Down[i],
                recovery_df$Missed_total[i], recovery_df$Recovery_pct[i]))
  }
  write.csv(recovery_df, "DEG_Recovery_Summary.csv", row.names = FALSE)

  # --- Stacked bar: category breakdown of missed DEGs per construct ---
  detail_all <- do.call(rbind, lapply(missed_detail, as.data.frame))
  if (!is.null(detail_all)) rownames(detail_all) <- NULL

  if (!is.null(detail_all) && nrow(detail_all) > 0) {
    ct <- as.data.frame(table(Construct = detail_all$Construct,
                               Category  = detail_all$Category))
    ct <- ct[ct$Freq > 0, ]
    cat_counts <- ct
    names(cat_counts)[names(cat_counts) == "Freq"] <- "n"
    ...
    cat("\n=== detail_all diagnostics ===\n")
    cat("nrow:", nrow(detail_all), "\n")
    cat("columns:", paste(colnames(detail_all), collapse = ", "), "\n")
    cat("joseph_lfc NA:", sum(is.na(as.numeric(detail_all$joseph_lfc))),
        "/", nrow(detail_all), "\n")
    cat("Category values:", paste(unique(detail_all$Category), collapse = ", "), "\n")

    cat("\n=== Median |Joseph LFC| for missed genes by category ===\n")
    detail_all$joseph_lfc_num <- as.numeric(detail_all$joseph_lfc)
    lfc_summary <- aggregate(
      abs(joseph_lfc_num) ~ Construct + Category,
      data      = detail_all,
      FUN       = function(x) round(median(x, na.rm = TRUE), 2),
      na.action = na.pass
    )
    names(lfc_summary)[3] <- "median_joseph_lfc"
    # Count per Construct+Category using plain table on the factor columns
    count_tbl <- table(detail_all$Construct, detail_all$Category)
    lfc_summary$n <- mapply(function(con, cat) {
      if (con %in% rownames(count_tbl) && cat %in% colnames(count_tbl))
        as.integer(count_tbl[con, cat])
      else
        NA_integer_
    }, lfc_summary$Construct, lfc_summary$Category)
    cat("lfc_summary nrow:", nrow(lfc_summary), "\n")
    cat(sprintf("%-10s  %-25s  %18s  %5s\n",
                "Construct", "Category", "Median_Joseph_LFC", "n"))
    cat(strrep("-", 65), "\n")
    for (i in seq_len(nrow(lfc_summary))) {
      n_val <- lfc_summary$n[i]
      cat(sprintf("%-10s  %-25s  %18.2f  %s\n",
                  lfc_summary$Construct[i],
                  lfc_summary$Category[i],
                  lfc_summary$median_joseph_lfc[i],
                  if (is.na(n_val)) "   NA" else sprintf("%5d", n_val)))
    }
    write.csv(lfc_summary, "DEG_Recovery_LFC_Summary.csv", row.names = FALSE)
  }

} else {
  if (is.null(joseph_clusters_rec)) {
    cat("DEG recovery analysis: reference file not found. Place one of these in the working directory:\n")
    cat(" ", paste(deg_rec_candidates, collapse = "\n  "), "\n")
  } else if (!exists("de_results_list")) {
    cat("DEG recovery analysis: de_results_list not found — re-run the differential_expression chunk.\n")
  } else {
    cat("DEG recovery analysis: 'Geneid' column not found in reference file.\n")
    cat("Columns found:", paste(colnames(joseph_clusters_rec), collapse = ", "), "\n")
  }
}
```
