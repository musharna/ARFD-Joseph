...
if (!is.na(joseph_file) && exists("joseph_clusters") &&
  ...
  # --- Auto-detect mapping: find which Joseph Up_Down col matches each of our constructs ---
  # Strip "Up_Down-" prefix from both sets, then try to fuzzy-match
  j_suffixes <- sub("^Up_Down-", "", ud_j)   # e.g. GUSaux_A, 5D_A, ...
  m_suffixes <- sub("^Up_Down-", "", ud_m)   # e.g. GUS_aux, 5D, ...

  # Build mapping by stripping non-alphanumeric chars and matching case-insensitively
  strip <- function(x) tolower(gsub("[^a-zA-Z0-9]", "", x))

  construct_map <- do.call(rbind, lapply(treatment_constructs, function(con) {
    con_stripped <- strip(con)
    # find best match in joseph's suffixes
    match_idx <- which(strip(j_suffixes) == con_stripped)
    if (length(match_idx) == 0) {
      # fallback: partial match (e.g. "gusaux" in "gusauxa")
      match_idx <- which(startsWith(strip(j_suffixes), con_stripped) |
                         startsWith(con_stripped, strip(j_suffixes)))
    }
    if (length(match_idx) > 0) {
      data.frame(my_code  = con,
                 j_suffix = j_suffixes[match_idx[1]],
                 stringsAsFactors = FALSE)
    } else {
      data.frame(my_code  = con,
                 j_suffix = NA_character_,
                 stringsAsFactors = FALSE)
    }
  }))
  ...
  for (i in seq_len(nrow(construct_map))) {
    ...
    if (!is.null(full_res)) {
      ...
      full_res <- full_res %>%
        rename(my_lfc  = all_of(lfc_col_full),
      ...
      detail <- full_res %>%
        ...
        mutate(
          ...
          Category = case_when(
            ...
            abs(my_lfc) < lfc_threshold                ~ paste0("LFC below threshold\n(|LFC| < ", lfc_threshold, ")"),
            TRUE                                       ~ "Other"
          )
        )
  ...
  if (!is.null(detail_all) && nrow(detail_all) > 0) {
    ...
    scatter_data <- detail_all %>%
      filter(!is.na(my_lfc) & !is.na(joseph_lfc))
    ...
    detail_all %>%
      group_by(Construct, Category) %>%
      summarise(n = n(),
                median_joseph_lfc = round(median(abs(joseph_lfc), na.rm = TRUE), 2),
                .groups = "drop") %>%
      print()
  }
