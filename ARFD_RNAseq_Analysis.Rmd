...
if (!is.na(joseph_file) && exists("joseph_clusters") &&
  ...
  if (!is.null(detail_all) && nrow(detail_all) > 0) {
    ...
    lfc_summary$n <- as.integer(table(
      paste(detail_all$Construct, detail_all$Category)
    )[paste(lfc_summary$Construct, lfc_summary$Category)])
