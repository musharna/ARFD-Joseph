...
if (!is.na(joseph_file) && exists("joseph_clusters") &&
  ...
  # Map OUR construct codes → Joseph's column suffix and display label
  # Our codes:   GUS_aux, 5D, 6D, 7D, 8D, 19D
  # Joseph cols: GUSaux_A, 5D_A, 6D_A, 7D_A, 8D_A, 19D_A
  construct_map <- data.frame(
    my_code  = treatment_constructs,
    j_suffix = c("GUSaux_A", "5D_A", "6D_A", "7D_A", "8D_A", "19D_A"),
    label    = c("GUS_aux",  "ARF5Δ","ARF6Δ","ARF7Δ","ARF8Δ","ARF19Δ"),
    stringsAsFactors = FALSE
  )

  # Normalise Geneids for matching
  ...
  for (i in seq_len(nrow(construct_map))) {
    ...
    con_label <- construct_map$label[i]
    ...
    if (!j_col %in% colnames(joseph_clusters)) {
      cat("Column not found in Joseph's file:", j_col, "\n")
      next
    }
